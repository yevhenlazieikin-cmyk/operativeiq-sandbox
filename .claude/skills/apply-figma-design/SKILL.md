...
- Every page opens with `bo-header` and closes with `bo-footer` (see CLAUDE.md §Page frame). The root `<section>` gets `margin-top: 40px`. Provide `MockMenuService` at the component level and a `siteInfo` signal typed as `SiteInfo`.
- Headers: ALWAYS `<bo-action-buttons-panel [title] [buttons] [state]="menuType.operation | menuType.administration" />` inside the `<section>` — never hand-rolled markup.
