...
## Page frame (non-negotiable)

Every sandbox page **must** open with `bo-header` and close with `bo-footer`:
...
- `margin-top: 40px` on the page's root `<section>` provides the required gap to `bo-action-buttons-panel`.
- Override `MenuService` with `MockMenuService` at the component level:
  `providers: [{ provide: MenuService, useClass: MockMenuService }]`
- `MockMenuService` lives at `src/app/pages/<name>/mock-menu.service.ts` and imports from `@mocks/navigation-menu`.
- Shared nav data lives at `src/app/__mocks__/navigation-menu.ts` — reuse across pages, never duplicate.
- `siteInfo` is a component signal typed as `SiteInfo` (deep-import `@backoffice/shared-ui/lib/header/site-info.interface`).
