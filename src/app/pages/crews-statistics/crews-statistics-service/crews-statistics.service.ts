...
export class CrewsStatisticsService {
  getSummary(): Observable<CrewSummary> {
    return of({ totalCrews: 12, activeCrews: 8, inactiveCrews: 4, totalMembers: 47, averageCrewSize: 5.9 });
  }

  getPerformance(): Observable<CrewPerformance> {
    return of({ tasksCompletedThisMonth: 284, avgResponseTimeMin: 7.2, incidentResolvedRatePct: 94.6, trainingCompletionRatePct: 88.3 });
  }
  ...
  getMembers(): Observable<CrewMember[]> {
    return of([
      ...
      { id: 5, name: 'Alex Muller', role: 'Firefighter', station: 'Station 9', country: 'Germany', tasksCompleted: 22, avgResponseTimeMin: 9.1 },
