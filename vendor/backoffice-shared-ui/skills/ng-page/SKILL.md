...
Action buttons panel is the page header — do not hand-roll a header. The `bo-layout` shell in the app root wraps the outlet, so the page renders inside it.
...
<!-- page body depends on kind — see per-kind sections -->
```

Use `menuType.operation` or `menuType.administration` for `[state]` to match the route's `data.menuType`.
